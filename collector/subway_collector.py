...
핵심 제약 (§3.3.2):
  - 키당 1,000회/일 하드 리밋 → DAILY_CAP(950) 으로 방어. 키를 여러 개 두면 라운드로빈
    (현재 5키를 라운드로빈 사용하고 잔여 쿼터에 따라 보통 30~50초 간격)
  - 갱신 10~20초. 최소 간격은 30초이며 실제 간격은 쿼터에 맞춰 자동 조정한다
...
# 갱신주기(10~20초)보다 성기게 — 최소 간격. 실제 간격은 키 수·남은 쿼터·남은
# 운행시간으로 pace()가 매 사이클 계산한다. 30초 하한이면 5키에서 보통 40~50초,
# 키가 적으면 쿼터를 하루에 고르게 쓰도록 더 긴 간격이 자동으로 나온다.
INTERVAL_SEC = 30
...
def resolve_keys():
    ...
    키 N개 = 950N 콜/일 ÷ 3 = 316N 회/일. 5키면 보통 약 40~50초 간격.
    """
