...
def main():
    ...
    ap.add_argument("--arex", help="공항철도 xlsx 경로")
    ap.add_argument("--lrt", nargs="*", default=[], help="경전철 텍스트: '노선명=경로' (신림선·우이신설선)")
    ...
    args = ap.parse_args()
    if not any((args.stcs, args.sinbundang, args.xlsx, args.gtxa, args.arex, args.lrt)):
        sys.exit("계획 소스가 없다 — --stcs · --sinbundang · --xlsx · --gtxa · --arex · --lrt 중 하나는 필요.")

    xlsx_specs = [tuple(s.split("=", 1)) for s in args.xlsx if "=" in s]
    lrt_specs = [tuple(s.split("=", 1)) for s in args.lrt if "=" in s]
    ...
    if xlsx_specs or args.gtxa or args.arex or lrt_specs:
