...
| | 한도 | 현재 |
|---|---|---|
| 버스위치정보 (TAGO 운영) | **키당 50만/일** | 키 2개·최대 340노선, 키별 소프트캡 48만. 대시보드에 실제 키별 호출량 표시 |
...
경기 버스는 340노선을 한 덩어리로 기다리는 배치 사이클이 아니라 **노선별 독립
31초 주기**로 요청한다. 30초 실측 처리량을 24시간 환산하면 두 키 합계 cap 96만을
약 1.6% 넘었기 때문에, 하루 끝 수집 중단 없이 쿼터를 약 98% 활용하도록 31초로
조정했다. 각 키의 제출은 계속 `dispatchRate`로 균등 제한하고
